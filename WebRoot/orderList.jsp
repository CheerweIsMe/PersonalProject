...
<script type="text/javascript">
	function doDelete(oid) {
		...
		if (flag)
			window.location.href = "${pageContext.request.contextPath}/delOrder.do?oid="
					+ oid;
	}
	
	function ViewOrderDetails(uid,oid){
		window.location.href = "${pageContext.request.contextPath}/selectOneOrder.do?oid="
			+ oid+"&id="+uid;
	}
</script>
</head>
<body class="animsition">
	<div class="wish-list" id="page">
		...
		<section class="sub-header shop-detail-1">
			...
			<h3 class="heading-style-3">订单列表</h3>
		</section>
						...
						<table name="" class="woocommerce-cart-table">
							<thead>
								<tr>
									...
									<th class="product-price">下单时间</th>
									<th class="product-status">总价格</th>
								...
								<c:forEach items="${allOrder}" var="o">
									<tr>
										...
										<td class="product-name" data-title="Product"><a
											class="product-name" href="shop-detail.jsp">${o.oid }</a>
										</td>
										<td class="product-price" data-title="Weight"></td>
										<td class="product-price" data-title="Weight">${o.ordertime }</td>
										<td class="product-stock-status" data-title="Stock Status">
											${o.total}</td>

										<td class="product-add-to-cart"><a class="btn btn-brand" onclick="ViewOrderDetails(${u.uid},${o.oid})"
											href="#">查看订单详情</a></td>
									<%-- 	<td class="product-remove text-right"><a class="remove"
											href="javascript:doDelete(${o.oid})"
